...
func (m *RWMutex) LockNamedTimeout(name string, timeout time.Duration) (err error) {
	...
	case <-m.writeLock: // Grab the ticket
		if debugLocks {
			log.Printf("(RWMutex/LockNamed %s) got lock", name)
		}
	...
	if debugLocks {
		log.Printf("(%s) LOCKED by %s", m.Name, name)
	}
	return

}

// LockNamed For unlock use Unlock()
...
func (m *RWMutex) Unlock() {
	...
	if debugLocks {
		log.Printf("(%s) UNLOCKED by %s **********************", m.Name, oldOwner)
	}
}
...
func (m *RWMutex) RLockNamedTimeout(name string, timeout time.Duration) (rl ReadLock, err error) {
	if debugLocks {
		log.Printf("(%s, %s) request RLock and Lock.", m.Name, name)
	}
	if m.Name == "" {
	...
	if debugLocks {
		log.Printf("(%s) RLock/%s got Lock.", m.Name, name)
	}
	m.readLock.Lock()
...
func (m *RWMutex) RUnlockNamed(rl ReadLock) {
	...
	if debugLocks {
		log.Printf("(%s) request RUnlock.", m.Name)
	}
	m.readLock.Lock()
	//m.readCounter -= 1
	...
	if ok {
		...
		if debugLocks {
			log.Printf("(%s) %s did RUnlock.", m.Name, name)
		}
	} else {
		log.Printf("(%s) ERROR: %s did not have RLock !?!??!?!?.", m.Name, name)
	}
	m.readLock.Unlock()
	if debugLocks {
		log.Printf("(%s) did RUnlock.", m.Name)
	}
}
